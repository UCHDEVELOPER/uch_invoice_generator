...
import { jobImportQueue } from "../workers/job-import/jobImport.queue.js";
import fs from "fs";
...
export async function importJobs(req, res) {
  try {
    ...
    await jobImportQueue.add("importJobs", {
      filePath: filePath,
...
export async function exportJobs(req, res) {
  try {
    ...
    const result = await exportJobsService(start_date, end_date);
