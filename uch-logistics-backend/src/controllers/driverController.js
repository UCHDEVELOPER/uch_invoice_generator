...
import {
  ...
  deleteCarryForwardService,
} from "../services/driverService.js";
import { exportDriversService } from "../utils/exportDriversPDF.js";
import validateDriverPositionPayload from "../helpers/driverPositionValidator.js";

export async function addDriver(req, res) {
  try {
    const payloadCheck = validateDriverPayload(req.body);
...
export async function exportDrivers(req, res) {
  try {
    ...
    const result = await exportDriversService(format, columns);
    return res.status(result.statusCode).json(result);
  } catch (err) {
    return res.status(500).json({
      success: false,
...
export async function deleteCarryForwardCharges(req, res) {
  try {
    const { id } = req.params;
    ...
    const result = await deleteCarryForwardService(id);
    return res.status(result.statusCode).json(result);
  } catch (err) {
    return res.status(500).json({
      success: false,
