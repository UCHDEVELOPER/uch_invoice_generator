...
export async function bulkRedraftInvoiceService(invoiceIds) {
  const redrafted = [];
  const failed = [];
  ...
  for (const invoiceId of invoiceIds) {
    try {
      ...
      if (result.success) {
        redrafted.push({ invoiceId, data: result.data });
      } else {
  ...
  const statusCode =
    failed.length === 0 ? 200 : redrafted.length === 0 ? 500 : 207; // 207 = partial success
  ...
  return {
    ...
    message:
      failed.length === 0
        ? `Successfully redrafted ${redrafted.length} invoice(s)`
        : `Redrafted ${redrafted.length} invoice(s), ${failed.length} failed`,
    data: {
      total: invoiceIds.length,
      successCount: redrafted.length,
